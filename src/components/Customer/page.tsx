...
export default function DemoPage({ customersinfo, customers }: DemoPageProps) {
  // Accept customersinfo as prop
    ...
    <div className="w-full bg-white p-4">
      ...
      session?.user.role === "business admin" ? (
        <div className="w-full flex items-center gap-2 justify-end">
          <Button
            className="mt-2 bg-primary700 flex justify-items-center gap-2"
      ...
      <div className="p-4">
        <Suspense
          fallback={<Loader className="animate animate-spin text-primary800" />}
        >
          <DataTable columns={columns} data={customers} />
        </Suspense>
      </div>
      {isOpen && <CustomerForm />}{" "}
      {/* Render the form only if isOpen is true */}
    </div>
  );
}
