...
export default function CreateJob() {
  ...
  const fetchCustomers = async () => {
    try {
      const response = await fetch(
        baseUrl + `customers/${session?.user?.companyId}`,
        {
          headers: {
            Authorization: `Bearer ${session?.user?.access_token}`,
          },
  ...
  const fetchEmployees = async () => {
    try {
      const response = await fetch(
        baseUrl + `${session?.user?.companyId}/employees`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${session?.user?.access_token}`,
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Textarea
                id="otherInfo"
                name="otherInfo"
                value={formData.location.otherInfo}
                onChange={handleLocationChange}
              />
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select
                name="dispatcher"
                onValueChange={(value) =>
                  setFormData((prev) => ({ ...prev, dispatcher: value }))
                }
              >
                <SelectTrigger>
          ...
          <div className="space-y-4">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    ...
                    {formData.jobSchedule.startDate
                      ? format(formData.jobSchedule.startDate, "PPP")
                      : "Pick a date"}
                  </Button>
                </PopoverTrigger>
            ...
            <div>
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    <CalendarIcon className="mr-2 h-4 w-4" />
                    {formData.jobSchedule.endDate
                      ? format(formData.jobSchedule.endDate, "PPP")
                      : "Pick a date"}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0">
            ...
            <div>
              ...
              <Select
                name="recurrence"
                onValueChange={(value) =>
                  handleScheduleChange("recurrence", value)
                }
              >
                <SelectTrigger>
