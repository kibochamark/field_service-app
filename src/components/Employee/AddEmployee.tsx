...
const AddEmployee: React.FC<AddEmployeeProps> = ({ roles }) => {
    ...
    const validateForm = () => {
        const { firstname, lastname, phonenumber, email, password, roleId, permissions } = formData;

        if (!firstname || !lastname || !phonenumber || !email || !password || !roleId || permissions.length === 0) {
            toast.error('All fields are required and permissions should have at least one value');
            return false;
        }
    ...
    return (
        <Card className="w-full max-w-2xl mx-auto">
            ...
            {roles.length > 0 ? (
                ...
                <CardContent className="space-y-4">
                    <div className="text-red-500">No roles available. Please contact your administrator.</div>
                </CardContent>
            )}
        </Card>
    );
